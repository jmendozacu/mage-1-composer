...
class Nwdthemes_Revslider_Helper_Products extends Mage_Core_Helper_Abstract {
	...
	public function getProductsByQuery($query) {
		...
		$storeId = Mage::app()->getStore()->getId();
		$productsCollection = Mage::getModel('catalog/product')->setStoreId($storeId)->getCollection();
	...
	private function _prepareProduct($product) {
		...
		$arrProduct['special_price'] = Mage::helper('core')->currency($product->getSpecialPrice(), true, false);
